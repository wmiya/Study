...
;( function( window ) {
	...
	UIProgressButton.prototype.stop = function() {
		var self = this,
			endLoading = function() {
				console.log(true);
				...
				if( typeof window.send === 'number' ) {
					var statusClass = window.send > 0 ? 'success' : 'error',
						statusEl = window.send > 0 ? self.successEl : self.errorEl;
